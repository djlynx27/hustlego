...
describe('getWeatherMultiplier', () => {
  ...
  it('returns 1.4 for heavy rain (weatherId 502-531)', () => {
    const weather: WeatherCondition = { weatherId: 502, temp: 5, precipMm: 15, windKph: 20, condition: 'rain' };
    expect(getWeatherMultiplier(weather)).toBe(1.4);
  });

  it('returns 1.4 for snow (weatherId 600-622)', () => {
    const weather: WeatherCondition = { weatherId: 601, temp: -5, precipMm: 5, windKph: 10, condition: 'snow' };
    expect(getWeatherMultiplier(weather)).toBe(1.4);
  });

  it('returns 1.4 for thunderstorm (weatherId 200-232)', () => {
    const weather: WeatherCondition = { weatherId: 210, temp: 15, precipMm: 20, windKph: 30, condition: 'storm' };
    expect(getWeatherMultiplier(weather)).toBe(1.4);
  });

  it('returns 1.15 for light rain (weatherId 300-501)', () => {
    const weather: WeatherCondition = { weatherId: 300, temp: 8, precipMm: 3, windKph: 5, condition: 'drizzle' };
    expect(getWeatherMultiplier(weather)).toBe(1.15);
  });

  it('returns 1.25 for extreme cold (temp < -15)', () => {
    const weather: WeatherCondition = { weatherId: 800, temp: -20, precipMm: 0, windKph: 0, condition: 'clear' };
    expect(getWeatherMultiplier(weather)).toBe(1.25);
  });

  it('returns 1.0 for clear mild weather', () => {
    const weather: WeatherCondition = { weatherId: 800, temp: 15, precipMm: 0, windKph: 5, condition: 'clear' };
...
describe('scoreAllZones', () => {
  const zones = [
    { id: 'z1', name: 'Plateau', type: 'résidentiel', latitude: 45.52, longitude: -73.58, city_id: 'mtl', created_at: '', demand_weight: 1, display_order: 1, is_active: true, radius_km: 2 },
    { id: 'z2', name: 'Aéroport YUL', type: 'aéroport', latitude: 45.47, longitude: -73.74, city_id: 'mtl', created_at: '', demand_weight: 1, display_order: 2, is_active: true, radius_km: 5 },
  ];

  it('scores all provided zones returning a Map per zone id', () => {
    const { scores } = scoreAllZones(zones, new Date('2026-03-21T14:00:00'), null, []);
  ...
  it('returns a factors Map with an entry per zone', () => {
    const { factors } = scoreAllZones(zones, new Date('2026-03-21T14:00:00'), null, []);
