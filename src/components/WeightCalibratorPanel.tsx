        ...
        {lastResult && (
          <div className="bg-muted/40 rounded-lg border border-border/50 p-3 text-[12px] font-body space-y-1">
            ...
            <p className="text-muted-foreground">
              {lastResult.trip_count} courses · MAE ${lastResult.mae.toFixed(2)}
              /h · précision {Math.round(lastResult.accuracy_pct)}%
            </p>
            {lastResult.message && (
                ...
                {history.map((h, i) => (
                  ...
                  >
                    ...
                    <td className="py-1 pr-3 text-right font-mono">
                      {h.mae !== null ? `$${h.mae.toFixed(1)}/h` : '—'}
                    </td>
                    <td className="py-1 pr-3 text-right font-mono">
                      {h.accuracy_pct !== null
                        ? `${Math.round(h.accuracy_pct!)}%`
                        : '—'}
                    </td>
                    <td className="py-1 pr-3 text-right font-mono">
                      {h.trip_count ?? '—'}
